...
$date = $_GET['date'] ?? '';
$field = $_GET['field'] ?? '';
$duration = isset($_GET['duration']) ? (int)$_GET['duration'] : 60; // predvolené trvanie: 60 minút
...
// Vygenerujeme všetky časové sloty od 08:00 do 22:00 - duration
$availableTimes = [];
...
for ($slot = $start; $slot <= $end; $slot += 15 * 60) {
    ...
    foreach ($reservations as $res) {
        if (($res['date'] ?? '') === $date && ($res['field'] ?? '') === $field) {
            ...
            // Kontrola prekrývania rezervácií
